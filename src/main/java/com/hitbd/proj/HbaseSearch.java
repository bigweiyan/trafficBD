...
public class HbaseSearch implements IHbaseSearch {
	...
	public void insertAlarm(List<IAlarm> alarms) throws TimeException, ForeignKeyException {
		for (IAlarm alarm : alarms) {
			...
			try {
				...
				SimpleDateFormat dateformatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				StringBuilder sb = new StringBuilder();
				sb.append('\"').append(alarm.getAddress()).append("\",").append(alarm.getEncId()).append(',')
						.append(alarm.getId()).append(',');
				sb.append(alarm.getLatitude()).append(',').append(alarm.getLongitude()).append(',')
						.append(dateformatter.format(alarm.getPushTime())).append(',').append(alarm.getVelocity());
	...
	public List<IAlarm> queryAlarmByUser(List<Integer> userBIds, boolean recursive, int sortType, QueryFilter filter) {
		...
		// }else {
		// directDevicesOfUserB.put(userBId, new
		// ArrayList<>(utilsObj.getdirectDevicesOfUserB(userBId)));
		// }
		...
		if (endTime.after(date48before)) {
			...
			while (!Utils.getTableName(mmddstartTime).equals(endTableName)) {
				for (Pair<String, String> pair : rowKeys) {
					try {
						Iterator<Result> results = utilsObj.scanTable(Utils.getTableName(mmddstartTime), pair.getKey(),
								pair.getValue(), connection);
						milliSecond = mmddstartTime.getTime() - Settings.BASETIME;
						period = (int) (milliSecond / (1000 * 60 * 60 * 24 * 4));
						utilsObj.addToList(results, startTime, endTime, queryResult,
								Settings.BASETIME + period * 1000L * 60 * 60 * 24 * 4);
						mmddstartTime = new Date(mmddstartTime.getTime() + 1000L * 60 * 60 * 24 * 4);
		...
		if (startTime.before(date48before)) {
			for (Pair<String, String> pair : rowKeys) {
				try {
					Iterator<Result> results = utilsObj.scanTable("alarm_history", pair.getKey(), pair.getValue(),
							connection);
					utilsObj.addToList(results, startTime, endTime, queryResult,
							Settings.BASETIME + period * 1000L * 60 * 60 * 24 * 4);
				} catch (IOException | ParseException e) {
		...
		/*
		 * sortType ==1 IMEI ==2 user_id ==3 createTime ==4 alarmType
		 */
			...
			for (HTableDescriptor oneTableDescriptor : allTable) {
				...
				for (Long imei : imeis) {
					try {
						...
						Iterator<Result> results = utilsObj.scanTable(oneTableName, pair.getKey(), pair.getValue(),
								connection);
	...
	@Override
	public void asyncQueryAlarmByUser(int qid, List<Integer> userBIds, boolean recursive, int sortType,
			QueryFilter filter) {
		IgniteSearch igniteSearchObj = new IgniteSearch();

		// 用户id为user_id的所有子用户
		// HashMap<Integer, ArrayList<Integer>> childrenOfUserB = new HashMap<Integer,
		// ArrayList<Integer>>();
